...
: ${K8S_TENANT_NAMESPACE:="skafos-kubevirt-kind"}
: ${K8S_TENANT_REALM:="realm-skafos-kubevirt-kind"}
: ${K8S_CLUSTER_NAME:="cluster-kubevirt-kind"}
: ${K8S_MASTER_NODES:="1"}
: ${K8S_WORKER_NODES:="1"}
...
if kubectl get namespace kubevirt 2>&1; then
...
if kubectl get namespace cdi 2>&1; then
...
until kubectl wait --timeout 300s --for=condition=ready pod -l cdi.kubevirt.io=cdi-operator -n cdi  2>/dev/null; do
echo CDI operator still not ready
sleep 10; 
done

echo "$(g_echo NAVARCOS:INFO:) Waiting for CDI"
until kubectl wait --timeout 300s --for=condition=ready pod -l cdi.kubevirt.io=cdi-apiserver -n cdi 2>/dev/null; do
echo CDI apiserver still not ready
sleep 10; 
done

echo "$(g_echo NAVARCOS:INFO:) Waiting for CDI"
until kubectl wait --timeout 300s --for=condition=ready pod -l     cdi.kubevirt.io=cdi-deployment -n cdi 2>/dev/null; do
...
if     kubectl get namespace kubevirt-manager 2>&1; then
...
envsubst < "./bootstrap_local_kubevirt/k8s-clusterapi-kubevirt-navarcos.TEMPLATE.yaml" > "./bootstrap_out/${K8S_TENANT_NAMESPACE}-${K8S_CLUSTER_NAME}.cluster.yaml"
envsubst < "./bootstrap_local_kubevirt/keycloak_realm.TEMPLATE.json" > "./bootstrap_out/${K8S_TENANT_NAMESPACE}-${K8S_CLUSTER_NAME}.keycloak.realm.json"

echo "$(g_echo NAVARCOS:INFO:) Creating clone permission for base image dataVolume on Infra-cluster"
if kubectl get rolebindings vm-${K8S_TENANT_NAMESPACE}-clone-permissions 2>&1; then
...
# kubectl get secret ${K8S_CLUSTER_NAME}-kubeconfig -n ${K8S_TENANT_NAMESPACE} --template={{.data.value}}|base64 -d > ./bootstrap_out/${K8S_TENANT_NAMESPACE}-${K8S_CLUSTER_NAME}.kubeconfig
...
while ! helm upgrade calico tigera-operator-navarcos --install --wait --namespace tigera-operator \
    ...
    --values ./bootstrap_external_kubevirt/calico-values.yaml; do
...
kind get kubeconfig --name navarcos> infra-kubeconfig
sed -i "/^\s*server:/c\    server: https://$CONTROLPLANEIP:6443" infra-kubeconfig



envsubst < ./bootstrap_local_kubevirt/cloud-config.TEMPLATE >  ./bootstrap_local_kubevirt/cloud-config
echo "$(g_echo NAVARCOS:INFO:) Installing CCM on Infra-cluster"
kubectl create secret generic infra-kubeconfig --from-file=./infra-kubeconfig --dry-run=client -o yaml | kubectl apply -f - -n ${K8S_TENANT_NAMESPACE}
kubectl create secret generic tenant-kubeconfig --from-file=./tenant-kubeconfig --dry-run=client -o yaml | kubectl apply -f - -n ${K8S_TENANT_NAMESPACE}
kubectl create secret generic cloud-config --from-file=./bootstrap_local_kubevirt/cloud-config --dry-run=client -o yaml | kubectl apply -f - -n ${K8S_TENANT_NAMESPACE}
rm infra-kubeconfig
rm tenant-kubeconfig
rm ./bootstrap_local_kubevirt/cloud-config
envsubst < ./bootstrap_local_kubevirt/kcc-deployment.TEMPLATE.yaml > ./bootstrap_out/${K8S_TENANT_NAMESPACE}-${K8S_CLUSTER_NAME}-kcc-deployment.yaml
#install CCM
kubectl apply -f ./bootstrap_out/${K8S_TENANT_NAMESPACE}-${K8S_CLUSTER_NAME}-kcc-deployment.yaml -n ${K8S_TENANT_NAMESPACE}
